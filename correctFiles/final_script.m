...
for as = 1:2
    ...
    for s = 1:3
        snr_ideal = s*5;
        [noisy_signal, snr_actual_g(as,s)] = addGaussianNoise(signal, snr_ideal, as);
    ...
    for s = 1:3
        snr_ideal = s*5;
        [noisy_signal, snr_actual_u(as,s)]  = addUniformNoise(signal, snr_ideal, as);
    ...
    for s = 1:3
        snr_ideal = s*5;
        [noisy_signal, snr_actual_e(as,s)] = addExponentialNoise(signal, snr_ideal, as);
    ...
    for s = 1:3
        snr_ideal = s*5;
        [noisy_signal, snr_actual_r(as,s)] = addRayleighNoise(signal, snr_ideal, as);
